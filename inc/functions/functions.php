<?php
if( !function_exists( 'get_option_mhrs_general' ) ){
	function get_option_mhrs_general( $option_name = null ){
		$option_value = '';
...
add_filter('mhrs_setting_tabs', 'yss_settings_tab' );
function yss_settings_tab( $tabs ){
	$tabs['yss_tab'] = 'YSS Tab';
	return $tabs;
}
function mhrs_paginate_links($query, $paged){
	$big = 999999999;
	$args	=	array(
...
function mhrs_noimage_url(){
	$no_img_url = MHRS_PLUGIN_URL . '/assets/img/no-image.png';
	return $no_img_url;
}
function mhrs_noimage_width(){
	$no_img_url = MHRS_PLUGIN_URL . '/assets/img/no-image.png';
	return 766;
}
function mhrs_noimage_height(){
	return 768;
}
function mhrs_ajax_loader_url(){
	$ajax_loader_url = MHRS_PLUGIN_URL . '/assets/img/mhrs-ajax-loader.gif';
	return $ajax_loader_url;
}
function mhrs_get_hotel_detail( $hotel_id ){
	...
	$data	=	array(
					...
					'featured_image'		=>	array(
													'thumbnail'	=>	array(
																		'url'			=>	mhrs_noimage_url(),
																		'width'			=>	mhrs_noimage_width(),
																		'height'		=>	mhrs_noimage_height(),
																		'is_resized'	=>	null
																	),
													'medium'	=>	array(
																		'url'			=>	mhrs_noimage_url(),
	...
	$address = get_post_meta( $data['id'], MHRS_MKP . 'address', true );
	...
	$city_id = get_post_meta( $data['id'], MHRS_MKP . 'city_id', true );
	$city_id = trim( $city_id );
	
	if( !empty( $city_id ) ){
		$data['city_id'] = $city_id;
		$data['city_detail'] = mhrs_get_city_detail( $city_id );
	}
	
	$city = get_post_meta( $data['id'], MHRS_MKP . 'city', true );
	$city = trim( $city );
	if( !empty( $city ) ){
		$data['city'] = $city;
	}
	...
	$state_id = get_post_meta( $data['id'], MHRS_MKP . 'state_id', true );
	$state_id = trim( $state_id );
	if( !empty( $state_id ) ){
		$data['state_id'] = $state_id;
		$data['state_detail'] = mhrs_get_state_detail( $state_id );
	}
	
	$state = get_post_meta( $data['id'], MHRS_MKP . 'state', true );
	$state_id = trim( $state );
	if( !empty( $state ) ){
		$data['state'] = $state;
	}
	
	$country_id = get_post_meta( $data['id'], MHRS_MKP . 'country_id', true );
	$country_id = trim( $country_id );
	if( !empty( $country_id ) ){
		$data['country_id'] = $country_id;
		$data['country_detail'] = mhrs_get_country_detail( $country_id );
	}
	
	$country = get_post_meta( $data['id'], MHRS_MKP . 'country', true );
	$country = trim( $country );
	if( !empty( $country ) ){
		$data['country'] = $country;
	}
	
	$postal_code = get_post_meta( $data['id'], MHRS_MKP . 'postal_code', true );
...
function mhrs_get_room_detail( $room_id ){
	...
	$hotel_id = get_post_meta( $data['id'], MHRS_MKP . 'hotel_id', true );
	$hotel_id = trim( $hotel_id );
	if( !empty( $hotel_id ) ){
		$data['hotel_id'] = $hotel_id;
	}
	
	$hotel = get_post_meta( $data['id'], MHRS_MKP . 'hotel', true );
	$hotel = trim( $hotel );
	if( !empty( $hotel ) ){
		$data['hotel'] = $hotel;
	}
	
	$charge_per_night = get_post_meta( $data['id'], MHRS_MKP . 'charge_per_night', true );
	$charge_per_night = trim( $charge_per_night );
	if( is_numeric( $charge_per_night ) ){
		$data['charge_per_night'] = $charge_per_night;
	}
	
	$max_adult_occupancy = get_post_meta( $data['id'], MHRS_MKP . 'max_adult_occupancy', true );
	...
	$max_children_occupancy = get_post_meta( $data['id'], MHRS_MKP . 'max_children_occupancy', true );
	...
	$total_rooms = get_post_meta( $data['id'], MHRS_MKP . 'total_rooms', true );
...
function mhrs_get_city_detail( $city_id ){
	$data	=	array(
					'id'					=>	null,
					'title'					=>	null,
					'state_id'				=>  null,
...
function mhrs_get_state_detail( $state_id ){
	$data	=	array(
					'id'					=>	null,
					'title'					=>	null,
					'country_id'			=>  null,
...
function mhrs_get_country_detail( $country_id ){
	$data	=	array(
					'id'					=>	null,
					'title'					=>	null,
					'long_description'		=>	null,
...
function mhrs_get_hotel_full_address( $hotel_id ){
	$address = get_post_meta( $hotel_id, MHRS_MKP . 'address', true );
	$address = trim( $address );
	...
	$city_id = get_post_meta( $hotel_id, MHRS_MKP . 'city_id', true );
	$city_id = trim( $city_id );
	...
	$city = get_post_meta( $hotel_id, MHRS_MKP . 'city', true );
	$city = trim( $city );
	if( !empty( $city ) ){
		$data['city'] = $city;
	}
	...
	$state_id = get_post_meta( $hotel_id, MHRS_MKP . 'state_id', true );
	$state_id = trim( $state_id );
	if( !empty( $state_id ) ){
		$data['state_id'] = $state_id;
		$data['state_detail'] = mhrs_get_state_detail( $state_id );
	...
	$state = get_post_meta( $hotel_id, MHRS_MKP . 'state', true );
	$state_id = trim( $state );
	if( !empty( $state ) ){
		$data['state'] = $state;
	}
	
	$country_id = get_post_meta( $hotel_id, MHRS_MKP . 'country_id', true );
	$country_id = trim( $country_id );
	if( !empty( $country_id ) ){
		$data['country_id'] = $country_id;
		$data['country_detail'] = mhrs_get_country_detail( $country_id );
	...
	$country = get_post_meta( $hotel_id, MHRS_MKP . 'country', true );
	$country = trim( $country );
	if( !empty( $country ) ){
		$data['country'] = $country;
	}
	
	$postal_code = get_post_meta( $hotel_id, MHRS_MKP . 'postal_code', true );
	$postal_code = trim( $postal_code );
	if( !empty( $postal_code ) ){
		$data['postal_code'] = $postal_code;
	}
...
function send_mail_via_wp_mail($to, $subject, $message, $from_mail = "")
{
	...
	add_filter( 'wp_mail_content_type', 'mhrs_set_html_content_type_func' );
	$headers = 'From: '.$from_mail.' <'.$from_mail.'>' . "\r\n";
	wp_mail( $to, $subject, $message, $headers );
	remove_filter( 'wp_mail_content_type', 'mhrs_set_html_content_type_func' );
}

function mhrs_set_html_content_type_func() 
{
	return 'text/html';
}

function mhrs_money_format( $price ) {
  return '$' . number_format( $price, 2 );
}

function mhrs_get_bookings_by_usersearch( $search_str ){
	...
	if( !empty( $search_arr ) ){
		...
		if( !empty( $user_ids ) ){
			$args	=	array(
							'post_type'			=>	array( 'mhrs_bookings' ),
							'post_status'		=>	array( 'publish' ),
							'posts_per_page'	=>	'-1',
							'fields'			=>	'ids',
							'meta_query'		=>	array(
														array(
															'key'		=>	MHRS_MKP . 'user_id',
															'value'		=>	$user_ids,
															'compare'	=>	'IN',
														),
													),
...
add_filter( 'posts_where', 'mhrs_booking_search_where' );
function mhrs_booking_search_where( $where ){
	global $pagenow, $wpdb;
	
	if(
		is_admin() &&
		$pagenow == 'edit.php' &&
		$_GET['post_type'] == 'mhrs_bookings' &&
		$_GET['s'] != ''
	)
	{
		$pids = mhrs_get_bookings_by_usersearch( $_GET['s'] );
		if( $pids ){
			$pids = implode(",", $pids );
			$where .= " OR ID IN ($pids)";
		}
