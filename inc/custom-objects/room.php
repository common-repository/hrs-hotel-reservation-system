<?php
if( !class_exists('MHRS_Room_Type') ){
	class MHRS_Room_Type{
		private static $_instance = null;
		private $post_type = null;
		private $post_rewrite_slug = null;
		
		private function __construct(){
			$this->post_type = MHRS_PREFIX . 'rooms';
		...
		function register_custom_post_type() {
			$labels = array(
				'name'					=>	_x( 'Rooms', 'Post Type General Name', 'mhrs' ),
				'singular_name'			=>	_x( 'Room', 'Post Type Singular Name', 'mhrs' ),
				'menu_name'				=>	__( 'Rooms', 'mhrs' ),
				'name_admin_bar'		=>	__( 'Rooms', 'mhrs' ),
				'parent_item_colon'		=>	__( 'Parent Room:', 'mhrs' ),
				'all_items'				=>	__( 'Rooms', 'mhrs' ),
				'add_new_item'			=>	__( 'Add New Room', 'mhrs' ),
				'add_new'				=>	__( 'Add New Room', 'mhrs' ),
				'new_item'				=>	__( 'New Room', 'mhrs' ),
				'edit_item'				=>	__( 'Edit Room', 'mhrs' ),
				'update_item'			=>	__( 'Update Room', 'mhrs' ),
				'view_item'				=>	__( 'View Room', 'mhrs' ),
				'search_items'			=>	__( 'Search Room', 'mhrs' ),
				'not_found'				=>	__( 'Room Not found', 'mhrs' ),
				'not_found_in_trash'	=>	__( 'Room Not found in Trash', 'mhrs' ),
			);
			...
			$args = array(
				'label'					=>	__( 'Rooms', 'mhrs' ),
				'description'			=>	__( 'A Room Type in a Hotel, like double, single, luxury etc.', 'mhrs' ),
				'labels'				=>	$labels,
		...
		function register_details( $post_type ) {
			add_meta_box(
				MHRS_PREFIX .'room_details',
				__( 'Room Details', 'mhrs' ),
		...
		function save_details( $post_id ){
			...
			$data['hotel']					=	get_post_meta( $post_id, MHRS_MKP . 'hotel', true );
			$data['charge_per_night']		=	get_post_meta( $post_id, MHRS_MKP . 'charge_per_night', true );
			$data['max_adult_occupancy']	=	get_post_meta( $post_id, MHRS_MKP . 'max_adult_occupancy', true );
			$data['max_children_occupancy']	=	get_post_meta( $post_id, MHRS_MKP . 'max_children_occupancy', true );
			$data['total_rooms']			=	get_post_meta( $post_id, MHRS_MKP . 'total_rooms', true );
			
			if ( isset( $_POST[ MHRS_MKP . 'hotel' ] ) ) {
				$hotel_id = sanitize_text_field( $_POST[ MHRS_MKP . 'hotel' ] );
				
				if( !empty( $hotel_id ) ){
					$data['hotel_id'] = $hotel_id;
					$data['hotel'] = MHRS_Misc::get_hotel( $hotel_id );
					$data['hotel'] = $data['hotel']['title'];
				}
			}
			
			if ( isset( $_POST[ MHRS_MKP . 'charge_per_night' ] ) ) {
				$charge_per_night = (float) sanitize_text_field( $_POST[ MHRS_MKP . 'charge_per_night' ] );
			...
			if ( isset( $_POST[ MHRS_MKP . 'max_adult_occupancy' ] ) ) {
				$max_adult_occupancy = (int) sanitize_text_field( $_POST[ MHRS_MKP . 'max_adult_occupancy' ] );
			...
			if ( isset( $_POST[ MHRS_MKP . 'max_children_occupancy' ] ) ) {
				$max_children_occupancy = (int) sanitize_text_field( $_POST[ MHRS_MKP . 'max_children_occupancy' ] );
			...
			if ( isset( $_POST[ MHRS_MKP . 'total_rooms' ] ) ) {
				$total_rooms = (int) sanitize_text_field( $_POST[ MHRS_MKP . 'total_rooms' ] );
			...
			update_post_meta( $post_id, MHRS_MKP . 'hotel_id', $data['hotel_id'] );
			update_post_meta( $post_id, MHRS_MKP . 'hotel', $data['hotel'] );
			update_post_meta( $post_id, MHRS_MKP . 'charge_per_night', $data['charge_per_night'] );
			update_post_meta( $post_id, MHRS_MKP . 'max_adult_occupancy', $data['max_adult_occupancy'] );
			update_post_meta( $post_id, MHRS_MKP . 'max_children_occupancy', $data['max_children_occupancy'] );
			update_post_meta( $post_id, MHRS_MKP . 'total_rooms', $data['total_rooms'] );
		...
		function render_details(){
			...
			$charge_per_night				=	(float) get_post_meta( $post_id, MHRS_MKP . 'charge_per_night', true );
			if( 0 <= $charge_per_night ){
				$data['charge_per_night'] = $charge_per_night;
			}
			
			$max_adult_occupancy			=	(int) get_post_meta( $post_id, MHRS_MKP . 'max_adult_occupancy', true );
			if( 0 <=  $max_adult_occupancy ){
				$data['max_adult_occupancy'] = $max_adult_occupancy;
			}
			
			$max_children_occupancy			=	(int) get_post_meta( $post_id, MHRS_MKP . 'max_children_occupancy', true );
			if( 0 <= $max_children_occupancy ){
				$data['max_children_occupancy'] = $max_children_occupancy;
			}
			
			$total_rooms					=	(int)get_post_meta( $post_id, MHRS_MKP . 'total_rooms', true );
			if( 0 <= $total_rooms ){
				$data['total_rooms'] = $total_rooms;
			}
			...
			<table class="form-table">
				<tbody>
					<tr>
						...
						<td>
							<select id="<?php echo MHRS_MKP . 'hotel'; ?>" name="<?php echo MHRS_MKP . 'hotel'; ?>"  required="required">
								<option value="">— <?php _e( 'Select Hotel', 'mhrs' ); ?> —</option>
								<?php if( $hotels = MHRS_Misc::get_hotels() ) { ?>
							...
							<br />
							<a target="_blank" href="<?php echo admin_url( 'post-new.php?post_type=' . MHRS_PREFIX . 'hotels' ); ?>">Add new hotel</a>
						</td>
					</tr>
					<tr>
						<th scope="row">
							<label for="<?php echo MHRS_MKP . 'charge_per_night'; ?>"><?php echo __( 'Charge per night', 'mhrs' ); ?> (USD) : </label>
						</th>
						<td>
							$<input type="number" id="<?php echo MHRS_MKP . 'charge_per_night'; ?>" name="<?php echo MHRS_MKP . 'charge_per_night'; ?>" required="required" value="<?php echo $data['charge_per_night']; ?>" min="0" step="any" />
						</td>
					</tr>
					
					<tr>
						<th scope="row">
							<label for="<?php echo MHRS_MKP . 'max_adult_occupancy'; ?>"><?php echo __( 'Max. adult occupancy', 'mhrs' ); ?>: </label>
						</th>
						<td>
							<input type="number" id="<?php echo MHRS_MKP . 'max_adult_occupancy'; ?>" name="<?php echo MHRS_MKP . 'max_adult_occupancy'; ?>" required="required" value="<?php echo $data['max_adult_occupancy']; ?>" min="0" />
						</td>
					</tr>
					
					<tr>
						<th scope="row">
							<label for="<?php echo MHRS_MKP . 'max_children_occupancy'; ?>"><?php echo __( 'Max. children occupancy', 'mhrs' ); ?>: </label>
						</th>
						<td>
							<input type="number" id="<?php echo MHRS_MKP . 'max_children_occupancy'; ?>" name="<?php echo MHRS_MKP . 'max_children_occupancy'; ?>" required="required" value="<?php echo $data['max_children_occupancy']; ?>" min="0" />
						</td>
					</tr>
					
					<tr>
						<th scope="row">
							<label for="<?php echo MHRS_MKP . 'total_rooms'; ?>"><?php echo __( 'Total rooms of this type', 'mhrs' ); ?>: </label>
						</th>
						<td>
							<input type="number" id="<?php echo MHRS_MKP . 'total_rooms'; ?>" name="<?php echo MHRS_MKP . 'total_rooms'; ?>" required="required" value="<?php echo $data['total_rooms']; ?>" min="0" />
						</td>
					</tr>
		...
		function custom_column_content( $column, $post_id ) {
			switch ( $column ) {
				case 'hotel' :
					$data = get_post_meta( $post_id , MHRS_MKP . 'hotel', true );
					$hotel_id = get_post_meta( $post_id , MHRS_MKP . 'hotel_id', true );
					$data = trim( $data );
					if( !empty( $data ) ){
