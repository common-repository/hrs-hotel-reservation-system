<?php
if( !class_exists( 'MHRS_Template_Loader' ) ){
	class MHRS_Template_Loader{
		...
		function load_templates( $template ){
			...
			$file_template = '';
			$view_rooms_page_id = get_option( 'mhrs_view_rooms_page_id', -1 );
			$view_hotels_page_id = get_option( 'mhrs_view_hotels_page_id', -1 );
			
			if ( is_single() && get_post_type() == MHRS_PREFIX . 'rooms' ) {
				$file_template = 'single-' . MHRS_PREFIX . 'rooms.php';
				$theme_template[] = $file_template;
				$theme_template[] = MHRS_OVERRIDE_TEMPLATEPATH . $file_template;
			} else if( is_single() && get_post_type() == MHRS_PREFIX . 'hotels' ){
				$file_template = 'single-' . MHRS_PREFIX . 'hotels.php';
				$theme_template[] = $file_template;
				$theme_template[] = MHRS_OVERRIDE_TEMPLATEPATH . $file_template;
			} else if ( is_page( $view_rooms_page_id ) || is_post_type_archive( MHRS_PREFIX . 'rooms' ) ) {
				
				$file_template 	= 'archive-' . MHRS_PREFIX . 'rooms.php';
				$theme_template[] = $file_template;
				$theme_template[] = MHRS_OVERRIDE_TEMPLATEPATH . $file_template;
			} else if ( is_page( $view_hotels_page_id ) || is_post_type_archive( MHRS_PREFIX . 'hotels' ) ) {
				
				$file_template 	= 'archive-' . MHRS_PREFIX . 'hotels.php';
				$theme_template[] = $file_template;
				$theme_template[] = MHRS_OVERRIDE_TEMPLATEPATH . $file_template;
			}
			...
			if ( $file_template ) {
				...
				if ( ! $template ) {
					$template = MHRS_TEMPLATEPATH . $file_template;
				}
			}
		...
		function global_requests(){
			add_action( 'admin_post_mhrshotelsearchsubmit', array($this, 'hotel_search_submit_handler' ) );
			add_action( 'admin_post_nopriv_mhrshotelsearchsubmit', array($this, 'hotel_search_submit_handler' ) );
			
			add_action( 'admin_post_mhrsviewroomsbyhotelsubmit', array($this, 'view_rooms_by_hotel_submit_handler' ) );
			add_action( 'admin_post_nopriv_mhrsviewroomsbyhotelsubmit', array($this, 'view_rooms_by_hotel_submit_handler' ) );
			
			add_action( 'admin_post_mhrsbookroomnow', array($this, 'book_rooms_now_submit_handler' ) );
			add_action( 'admin_post_nopriv_mhrsbookroomnow', array($this, 'book_rooms_now_submit_handler' ) );
			
			add_action( 'admin_post_mhrsaddtoroomcart', array($this, 'add_room_cart_submit_handler' ) );
			add_action( 'admin_post_nopriv_mhrsaddtoroomcart', array($this, 'add_room_cart_submit_handler' ) );
		}
		...
		function hotel_search_submit_handler(){
			...
			if( isset( $_POST['mhrs_post_id'] ) ){
				$post_id = trim( $_POST['mhrs_post_id'] );
				if( empty( $post_id ) ){
			...
			*/
			...
			if( !isset( $_POST['mhrs_hotel_search_submit'] ) ){
				wp_redirect( home_url() );
				exit;
			}
			...
			if( $form_errors['error'] ){
				$_SESSION['mhrs_error']['hotelsearchsubmit']	=	$form_errors;
			...
			$view_hotels_url = MHRS_Misc::view_hotels_page_url();
		...
		function view_rooms_by_hotel_submit_handler(){
			...
			*/
			...
			if( !isset( $_POST['mhrs_view_rooms_by_hotel'] ) ){
				wp_redirect( home_url() );
				exit;
			}
			...
			if( $form_errors['error'] ){
				$_SESSION['mhrs_error']['viewroomsbyhotelsubmit']	=	$form_errors;
				$goback = add_query_arg( array( 'error' => 1 ), $goback );
				wp_redirect( $goback );
				exit;
			}
			...
			$view_room_url = MHRS_Misc::view_rooms_page_url();
		...
		function book_rooms_now_submit_handler(){
			...
			*/
			...
			if( !isset( $_POST['mhrsbookroomnow'] ) ){
				wp_redirect( home_url() );
				exit;
			}
			...
			$hotel_search_page_url  = MHRS_Misc::hotel_search_page_url();
