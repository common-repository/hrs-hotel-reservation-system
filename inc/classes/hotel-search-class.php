<?php
if( !class_exists( 'MHRS_Room_Search' ) ){
	class MHRS_Room_Search{
		...
		function __construct( $room_id ){
			...
			$this->hotel_id = (int) get_post_meta( $this->room_id, MHRS_MKP . 'hotel_id', true );
			$this->total_rooms = (int) get_post_meta( $this->room_id, MHRS_MKP . 'total_rooms', true );
		...
		function get_remaining_rooms_on( $date ){
			$on_date_bookings = (int) get_post_meta( $this->room_id, MHRS_MKP . 'bookings_on_' . $date , true );
...
if( !class_exists( 'MHRS_Hotel_Search' ) ){
	class MHRS_Hotel_Search{
		...
		public function get_hotels_by_location( $in_ids = array(), $param = array() ){
			...
			if( !empty( $in_ids ) ){
				foreach( $in_ids as $in_id ){
					$meta_query[]	=	array(
											'key'	=>	MHRS_MKP . 'city_id',
											'value'	=>	$in_id,
											'compare'		=>	'=',
										);
					$meta_query[]	=	array(
											'key'	=>	MHRS_MKP . 'state_id',
											'value'	=>	$in_id,
											'compare'		=>	'=',
										);
					$meta_query[]	=	array(
											'key'	=>	MHRS_MKP . 'country_id',
											'value'	=>	$in_id,
											'compare'		=>	'=',
										);
				}
		...
		function get_available_hotels( $data ){
			...
			if( $available_rooms ){
				...
				foreach( $available_rooms as $ar ){
					$hotel_id = get_post_meta( $ar, MHRS_MKP . 'hotel_id', true );
		...
		function get_non_booked_rooms_by_hotel( $hotels, $daterange ){
			...
			if( $rooms ){
				foreach( $rooms as $room ){
					$rsobj = new MHRS_Room_Search( $room );
		...
		function get_all_non_booked_rooms( $location, $checkindate, $checkoutdate ){
			...
			MHRS_Misc::print_r($hotel_ids);
			if( $hotel_ids ){
				$args['post__in'] = array(631);
			}
			$posts	=	get_posts( $args );
			foreach( $posts as $p ){
				echo '<br />'.get_post_meta( $p, MHRS_MKP . 'hotel_id', true);
			}
			...
			if( $posts ){
				if( !empty( $posts ) && count( $posts ) ){
					...
					MHRS_Misc::print_r($available_rooms);
					if( $available_rooms ){
		...
		function is_the_room_available( $data ){
			...
			if( !$error ){
				$is_published = MHRS_Misc::is_published( $room_id );
				if( !$is_published ){
					$error = true;
				}
			}
		...
		function get_required_rooms( $roomid, $adult_stay, $children_stay, $checkindate, $checkoutdate ){
			...
			$robj = new MHRS_Room_Search( $roomid );
			
			$max_rooms_available = $robj->get_min_available_rooms( $dates );
			$max_adult_occupancy = (int) get_post_meta( $roomid, MHRS_MKP . 'max_adult_occupancy', true );
			$max_children_occupancy = (int) get_post_meta( $roomid, MHRS_MKP . 'max_children_occupancy', true );
		...
		function book_room_on( $room_id , $date, $required_room ){
			$current_bookings = (int ) get_post_meta( $room_id, MHRS_MKP . 'bookings_on_' . $date , true );
			$updated_bookings = ( $current_bookings + $required_room );
			update_post_meta( $room_id, MHRS_MKP . 'bookings_on_' . $date , $updated_bookings );
		}
	} /* end of class */
}