jQuery( function ( $ ) {
	$(document).ready(function(){
		/* mhrs_load_loations_js_obj , prefix 'mhrs_' can be change */
		/* mhrs_load_loations_js_obj.admin_ajax_url */
		var mkp = mhrs_load_loations_js_obj.mkp;
		var city = $( "#\\"+ mkp + "city" );
		var state = $( "#\\"+ mkp + "state" );
		...
		function load_state( city_id ){
			if( state.length ){
				...
				$.ajax({
					url: mhrs_load_loations_js_obj.admin_ajax_url,
					method: 'POST',
					dataType: 'json',
					data: {
						'action': 'mhrsajax_load_state_by_city_id',
						'city_id': city_id
					}
				}).done(function(r){
					if( r.success ){
		...
		function load_country( state_id ){
			if( country.length ){
				...
				$.ajax({
					...
					dataType: 'json',
					data: {
						'action': 'mhrsajax_load_country_by_state_id',
						'state_id': state_id
					}
				}).done(function(r){
					if( r.success ){
