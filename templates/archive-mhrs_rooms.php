<?php
/**
 * The template for displaying Archive for mhrs_rooms post type
...
if( isset( $_SESSION['hotel_search_form_data'] ) ){
	...
	if( !empty( $_SESSION['hotel_search_form_data'] ) ){
		...
		if(	isset(
			...
			$search_rooms_obj = new MHRS_Hotel_Search();
...
if ( $query->have_posts() ) {
	...
	while ( $query->have_posts() ) {
		$query->the_post();
		$id = get_the_ID();
		$rooms[$id] = mhrs_get_room_detail( $id );
	}
	$mhrs_pagination = mhrs_paginate_links( $query, $paged );
	wp_reset_postdata();
}
?>
<?php get_header(); ?>
